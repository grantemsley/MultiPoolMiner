...
try {
    $Pool_Request = Invoke-RestMethod "http://www.BlockMasters.co/api/status" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
...
$Pool_Currencies = @("BTC", "DOGE", "LTC") + ($PoolCoins_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name) | Select-Object -Unique | Where-Object {Get-Variable $_ -ValueOnly -ErrorAction SilentlyContinue}
