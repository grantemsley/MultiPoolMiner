...
param(
    ...
    [Parameter(Mandatory = $false)]
    [Array]$ExcludeWatchdogAlgorithm = @("X16R"), #Do not use watchdog for these algorithms, e.g due to it's nature X16R will always trigger watchdog
    [Parameter(Mandatory = $false)]
    ...
    [Parameter(Mandatory = $false)]
    [Switch]$UseFastestMinerPerAlgoOnly #Use only use fastest miner per algo and device index. E.g. if there are 2 miners available to mine the same algo, only the faster of the two will ever be used, the slower will also be hidden in the summary screen
)
...
$WalletDonate = @("1BLXARB3GbKyEg8NTY56me5VXFsX2cixFB", "1GPSq8txFnyrYdXL8t6S94mYdF8cGqVQJF")
$UserNameDonate = @("grantemsley", "uselessguru")
$WorkerNameDonate = "multipoolminer"
$DonateDistribution = 0,1 #1:1
...
#    }
    ...
    Get-ChildItem "Miners" -File | Where-Object {-not $Config.Miners.($_.BaseName)} | ForEach-Object {
    ...
    if ($Timer.AddDays(-1).AddMinutes($Config.Donate) -ge $LastDonated) {
        Write-Log "Donation run, mining to donation address for the next $(($LastDonated - ($Timer.AddDays(-1))).Minutes +1) of $($Config.Donation) minutes. Note: MPM will use ALL available pools. "
        Get-ChildItem "Pools" -File | ForEach-Object {
            $Config.Pools | Add-Member $_.BaseName (
                [PSCustomObject]@{
                    BTC    = $WalletDonate[(Get-Random -InputObject $DonateDistribution)]
                    User   = $UserNameDonate[(Get-Random -InputObject $DonateDistribution)]
    ...
    #Use only use fastest miner per algo and device index. E.g. if there are 2 miners available to mine the same algo, only the faster of the two will ever be used, the slower sill also be hidden in the summary screen
    ...
    $Miners | Where-Object {$_.Profit -ge 1E-5 -or $_.Profit -eq $null} | Sort-Object -Descending Type, Device, Profit_Bias | Format-Table -GroupBy Type (
