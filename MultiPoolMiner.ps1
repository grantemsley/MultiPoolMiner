...
while ($true) {
    ...
    $AllMiners = if (Test-Path "MinersLegacy") {
        Get-ChildItemContentParallel "MinersLegacy" -Parameters @{Pools = $Pools; Stats = $Stats; Config = $Config; Devices = $Devices} | ForEach-Object {$_.Content | Add-Member Name $_.Name -PassThru -Force} |
