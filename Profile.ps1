...
while ($true) {
	...
	time1 "add pool watchdog timers"
    ...
    $API.Miners = $Miners
	time1 "reset previous activeminers"
	...
	time1 "dont penalize running miners"
	...
	time1 "bestminers_combo sorting"
	...
	time1 "bestminers_combo_comparison sorting"
    ...
    if (($BestMiners_Combo | Where-Object Profit -EQ $null | Measure-Object).Count -eq 0 -and $Downloader.State -ne "Running") {
        ...
        $MinerComparisons | Out-Null
    }

    #Display benchmarking progress
	time1 "display benchmarks"
