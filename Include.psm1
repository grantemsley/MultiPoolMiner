...
Function Get-BittrexMarkets {
    ...
    if(Test-Path $filename) {
        ...
        if(((Get-Date) - $lastupdated) -lt $timespan) {
            Write-Verbose 'Using cached market list...'
            $markets = Get-Content $filename | ConvertFrom-Json
            return $markets
        }
    } 

    Write-Verbose 'Updating markets from API...'
    try {
        $Request = Invoke-RestMethod "http://bittrex.com/api/v1.1/public/getmarkets" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
    } catch {
        Write-Warning "Bittrex exchange API has failed. "
    }
    
    $markets = $Request.result | Where-Object {$_.BaseCurrency -eq 'BTC'}
    Write-Verbose "$($markets.count) markets loaded"
...
function Get-BTCValue {
    ...
    if($ExchangeRates[$altcoin].LastUpdated -and ((Get-Date) - $ExchangeRates[$altcoin].LastUpdated) -lt $timespan) {
        Write-Verbose "Using cached $altcoin exchange rate from $($ExchangeRates[$altcoin].LastUpdated)"
    ...
    if($markets.count -eq 0) {
        Write-Warning "No market found for $altcoin, unable to get exchange rate"
        Return 0
    }

    # Get the exchange rate
    Write-Verbose "Updating exchange rate for $altcoin"
    try {
        $Request = Invoke-RestMethod "http://bittrex.com/api/v1.1/public/getticker?market=$($markets[0].MarketName)" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
    } catch {
        Write-Warning "Bittrex exchange API has failed."
    }
...
function Get-Balances {
    ...
    Write-Verbose 'Getting balances...'
